PDSB.MergedCalendar = (function () {
    ...
    var mergedCal = '[data-mergedCal]',
        ...
        getCalSettings = function () {
            $.ajax({
                url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('CalendarSettings')/items?$expand=RotaryCal&$select=Title,Id,isDefault,ShowCal,isRotary,BgColor,FgColor,CalName,CalURL,RotaryCalId,RotaryCal/Title,RotaryCal/Name",
                type: "GET",
                dataType: "json",
                async: false,
                headers: {
        ...
        getsecCalSettings = function () {
            $.ajax({
                url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('RotaryCals')/items?$select=Title,Name,Id",
                type: "GET",
                dataType: "json",
                async: false,
                headers: {
        ...
        updateList = function(items){
            var i, item = {}, itemShowCal, itemRotaryCal;
            for(i=0; i<items.length; i++){
                itemShowCal = items[i].showCal, itemRotaryCal = items[i].rotaryCal;
                if(itemRotaryCal != ""){
                    item = {
                        __metadata : {type: "SP.Data.CalendarSettingsListItem"},
                        ShowCal : itemShowCal,
                ...
                $.ajax({  
                    url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('CalendarSettings')/items("+items[i].id+")",  
                    type: "POST",  
                    headers: { 
                        "Accept": "application/json;odata=verbose",
