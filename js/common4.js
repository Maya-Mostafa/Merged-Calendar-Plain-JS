PDSB.MergedCalendar = (function () {
    ...
    var mergedCal = '[data-mergedCal]',
        template = {
            ...
            graphCalForm: '<table border="0" cellpadding="7" cellspacing="0" width="100%"><colgroup><col width="20%"/><col width="80%"/></colgroup><tbody>'+
                            '<tr><td>Title</td><td>__gCalTitle__</td></tr>'+
                            '<tr><td>Location</td><td>__gCalLoc__</td></tr>'+
                            '<tr><td>Start Time</td><td>__gCalStart__</td></tr>'+
                            '<tr><td>End Time</td><td>__gCalEnd__</td></tr>'+
                            '<tr><td valign="top">Description</td><td>__gCalBody__</td></tr>'+
                            '<tr><td></td><td></td></tr>'+
        ...
        appConfig = {  
            auth: {  
                clientId: "2eb4dc90-8ac5-414b-ab9f-0be1be2c0b61", //EmpInfo
        ...
        getCalSettings = function () {
            $.ajax({
                url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('" + calOptions.calSettingsList + "')/items?$expand=RotaryCal&$select=Title,Id,isDefault,ShowCal,isRotary,BgColor,FgColor,CalName,CalURL,RotaryCalId,RotaryCal/Title,RotaryCal/Name",
                type: "GET",
                dataType: "json",
                async: false,
                headers: {
        ...
        getSecCalSettings = function () {
            $.ajax({
                url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('" + calOptions.rotarySettingsList + "')/items?$select=Title,Name,Id",
                type: "GET",
                dataType: "json",
                async: false,
                headers: {
        ...
        isGraphCal = function(calURL){
            return calURL.toString().toLowerCase().indexOf('graph.microsoft.com') != -1
        },
        updateList = function(items){
            var i, item = {}, itemShowCal, itemRotaryCal, metaListName = "SP.Data." + calOptions.calSettingsList + "ListItem";
            for(i=0; i<items.length; i++){
                ...
                $.ajax({  
                    url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('" + calOptions.calSettingsList + "')/items("+items[i].id+")",  
                    type: "POST",  
                    headers: { 
                        "Accept": "application/json;odata=verbose",
                ...
                if (data[i].isRotary){
                    for(j=0; j<dataSec.length;j++){
                        dpdOps += template.dlgDpdOp
                            ...
                            .replace('__isSelected__', data[i].RotaryCalId == dataSec[j].Id ? 'selected' : '')
                            .replace('__isSel__', data[i].RotaryCalId == dataSec[j].Id ? 'selected' : '');
                    } 
                    dpdHTML = template.dlgDpd
                        .replace('__Options__', dpdOps)
                        .replace('__isDisabled__', data[i].RotaryCal.Title != undefined && data[i].ShowCal == true ? '' : 'is-disabled');
                }
                $('.ms-Dialog-content').append(
                    template.dlgChkbox
                        .replace('__CalID__', data[i].Id)
                        .replace('__CalTitle__', data[i].Title)
                        .replace(/__isDisabled__/g, data[i].isDefault ? 'is-disabled' : '')
                        .replace('__isChkd__', data[i].ShowCal)
                        .replace('__hasDpd__', data[i].isRotary ? dpdHTML : '')
                );
            };            
        },
        calNames = function () {
            ...
            for (i = 0; i < data.length; i++) {
                if (data[i].ShowCal){
                    if(data[i].RotaryCalId !== null){ //Rotary Calendar
                        calList.push(data[i].RotaryCal.Title);
                    }else if (isGraphCal(data[i].CalURL)){ //Graph API Calendars
        ...
        getGraphCalsData = function(info, calURL, successCallback, failureCallback){
            var events = [], calInfo;
        ...
        displayCalendars = function (calArr, deptArr, deptBg, deptFg) {
            ...
            for (var i = 0; i < deptArr.length; i++) {
                var deptURL;
                if (isGraphCal(deptArr[i])){
                    deptURL = deptArr[i];
                }else{
                    deptURL = "/" + deptArr[i] + "/_api/Web/Lists/GetByTitle('" + calArr[i] + "')/items?$select=ID,Title,EventDate,EndDate,fAllDayEvent,fRecurrence,RecurrenceData&$top=1000";
                }
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function (info) {
                    console.log(info)
                    if(!isGraphCal(info.event._def.extendedProps._graphURL)){
                        ...
                        var graphHtmlElem = document.createElement('div');
                        graphHtmlElem.innerHTML = template.graphCalForm
                                                        .replace('__gCalTitle__', info.event._def.title)
                                                        .replace('__gCalLoc__', info.event._def.extendedProps._location)
                                                        .replace('__gCalStart__', formatDate(info.event.start))
                                                        .replace('__gCalEnd__', formatDate(info.event.end))
                                                        .replace('__gCalBody__', info.event._def.extendedProps._body);
                        PDSB.UI.OpenHtml('Event Properties', graphHtmlElem);
                    }
                },
