PDSB.MergedCalendar = (function () {
    ...
    var mergedCal = '[data-mergedCal]',
        ...
        getCalSettings = function () {
            $.ajax({
                url: getWebUrl() + "/_api/web/lists/GetByTitle('" + calOptions.calSettingsList + "')/items?$expand=RotaryCal&$select=Title,Id,CalendarType,ShowCal,BgColor,FgColor,CalName,CalURL,RotaryCalId,RotaryCal/Title,RotaryCal/Name",
                type: "GET",
                dataType: "json",
                async: false,
                headers: {
        ...
        getSecCalSettings = function () {
            $.ajax({
                url: getWebUrl() + "/_api/web/lists/GetByTitle('" + calOptions.rotarySettingsList + "')/items?$select=Title,Name,Id",
                type: "GET",
                dataType: "json",
                async: false,
                headers: {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                $('.ms-Dialog-content').append(
                    template.dlgChkbox
                        ...
                        .replace(/__isDisabled__/g, data[i].CalendarType == 'My School' ? 'is-disabled' : '')
                        .replace('__isChkd__', data[i].ShowCal)
                        .replace('__hasDpd__', data[i].CalendarType == 'Rotary' ? dpdHTML : '')
                );
            };            
        },
        calNames = function () {
            ...
            for (i = 0; i < data.length; i++) {
                if (data[i].ShowCal){
                    if(isRotaryCal(data[i].CalendarType)){ //Rotary Calendar
                        calList.push(data[i].RotaryCal.Title);
                    }else if (isGraphCal(data[i].CalendarType)){ //Graph API Calendars
        ...
        calTypes = function(){
            ...
            for (i = 0; i < data.length; i++) {
                if (data[i].ShowCal)
                    calTypes.push(data[i].CalendarType);
            }
            return calTypes;
        },
